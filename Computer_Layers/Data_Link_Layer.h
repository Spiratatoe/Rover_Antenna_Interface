...
// ----------------------------------------------------------------------
// Data Link Layer Specification for RAI PIF by Josh
//
...
inline std::vector<uint8_t> byteUnstuff(const std::vector<uint8_t>& input) {
    ...
    for (size_t i = 0; i < input.size(); ) {
        if (input[i] == 0xAA) {
            if (i + 1 >= input.size()) {
                // Incomplete escape sequence so break
                break;
            }
            ...
            if (nextByte == 0x05) {
                ...
                // Unexpected sequence so output the escape byte and continue
...
inline std::vector<uint8_t> framePacket(const std::vector<uint8_t>& networkPacket) {
    ...
    // Apply and append byte stuffing to the raw field.
...
// Deserializes a data link layer frame back into the original
