...
#ifndef ALL_LAYERS_DATA_LINK_H
#define ALL_LAYERS_DATA_LINK_H
...
//imports for testing only can be commented out for real runtime
#include <cassert>
#include <iostream>
...
// for code testing can be erased when done tests
void test_serialization_and_deserialization(const uint8_t *data, uint16_t length) {
    ...
    // Create a frame
    Frame frame = create_frame(data, length);
    uint8_t serializedData[MTU * 2]; // Allow room for stuffed bytes
    ...
    for (uint16_t i = 0; i < serializedLength; ++i) {
        ...
        if (result == DESERIALIZE_ERROR) {
            std::cerr << "Deserialization error at byte " << i << "\n";
            break;
        }
    ...
    if (deserializationSuccess) {
        assert(deserializer.frame.length == length);
        for (uint16_t i = 0; i < length; ++i) {
            assert(deserializer.frame.data[i] == data[i]);
        }
        std::cout << "Test passed!\n";
    } else {
        std::cerr << "Test failed: Deserialization incomplete\n";
    }
